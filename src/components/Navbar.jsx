...
const Navbar = () => {
  ...
  const menuItems = ['HOME', 'SERVICES', 'PROJECTS', 'PAGES', 'SHOP', 'BLOG', 'CONTACT'];
    ...
    <div className="flex justify-between items-center px-6 py-3 max-w-7xl mx-auto">
      ...
      <ul className="hidden md:flex space-x-6 font-medium">
        {menuItems.map((item) => (
          <li
            key={item}
            className="hover:text-[#E87722] transition-colors cursor-pointer"
          >
            {item}
          </li>
        ))}
      ...
      <div className="hidden md:block">
        <button className="border px-5 py-2 rounded-full bg-[#E87722] hover:bg-white hover:text-[#3B6029] transition font-semibold">
          GET IN TOUCH
        </button>
      </div>
    ...
    {isMobileMenuOpen && (
      <div className="md:hidden bg-[#1a1a1a] px-6 py-4 space-y-4 transition-all duration-300 ease-in-out">
        {menuItems.map((item) => (
          <div
            key={item}
            className="hover:text-[#E87722] cursor-pointer text-base font-medium"
          >
            {item}
          </div>
        ))}
